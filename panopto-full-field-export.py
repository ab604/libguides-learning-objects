...
class PanoptoClient:
    ...
    def export_full_recordings_to_csv(self, folder_id, output_file):
        print(f"\nStarting full export to {output_file}")
        try:
            ...
            for recording in recordings:
                # Add main recording fields
                fieldnames.update(recording.keys())
                
                # Add viewer details fields if present
                if 'viewer_details' in recording:
                    fieldnames.update(
                        f"viewer_details_{key}" for key in recording['viewer_details'].keys()
                    )
            
            # Remove 'viewer_details' and replace with specific fields
            if 'viewer_details' in fieldnames:
                fieldnames.remove('viewer_details')
            ...
            with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                for recording in recordings:
                    # Flatten viewer details
                    if 'viewer_details' in recording:
                        for key, value in recording['viewer_details'].items():
                            recording[f"viewer_details_{key}"] = value
...
if __name__ == "__main__":
    try:
        ...
        OUTPUT_FILE = "panopto_full_recordings.csv"
        ...
        client.export_full_recordings_to_csv(FOLDER_ID, OUTPUT_FILE)
